...
```docker title="/docker/Dockerfile.test"
...
services:
	test:
		...
		# waiting for it to be running on port 3000 and then run osnap
		command: npx http-server dist --port 3000 --silent & (wait-on tcp:3000 && osnap)
```